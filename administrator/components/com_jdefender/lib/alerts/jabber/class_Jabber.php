 ...
 * Version 0.9rc2
 * @copyright Copyright 2002-2007, Centova Technologies Inc.
 ...
 *	     - SRV lookups now supported if PEAR's Net::DNS package is installed
 *         (also courtesy of Edward Rudd)
 *       - Added experimental service discovery support (courtesy of noalwin)
 *       - Fixed bug in which large XML packets would be split and thus parsed
 *         incorrectly
 *
 *	v0.9rc1 (Aug 04, 2007)
...
$baseDir = JPATH_ROOT.DS.'administrator'.DS.'components'.DS.'com_jdefender'.DS.'lib'.DS.'alerts'.DS.'jabber';

require_once($baseDir.DS."class_ConnectionSocket.php");
require_once($baseDir.DS."class_XMLParser.php");
...
class Jabber {
	...
	var $_connector			= "JD_ConnectionSocket";
	...
	function Jabber($enable_logging = false) {
		...
		$this->xml = &new JD_XMLParser();
		if (!empty($_SERVER['REMOTE_ADDR'])) {
			$this->_remote_ip = $_SERVER['REMOTE_ADDR'];
		} else {
			$this->_remote_ip = '127.0.0.1';
		}
	}
	...
	function _unique_id($prefix) {
		...
		return $prefix."_" . md5(time() . $this->_remote_ip . $this->_unique_counter);
	}
	...
	function _log($msg,$level = 1) {
		if ($this->_use_log) {
//			if ($this->_log_flush) 
//				$this->_log_file = @fopen(dirname(__FILE__)."/log/logfile.txt","a");
//			
//			if ($this->_log_file) {
...
//		$this->_set_iq_handler("_on_xxx_result",$data_id); // we don't really need the result from this... do we?
	...
	function _handle_iq(&$packet) {
		...
		if (!empty($this->_iq_handlers[$iq_id])) {
...
//		$this->_log("BROWSE packet: ".var_export($packet,true));
		...
		if (($packet_type=="result") || ($packet_type=="set")) {
			...
			for ($a = 0; $a < $number_of_contacts; $a++)
			{
				...
				if (!($is_service && $this->handle_services_internally)) {
					...
					if (!empty($this->presence_cache[$u_jid])) {
						if (!$show || $is_new) {
	...
	function _receive() {
		...
		$iterations = 0; 
		$empties = 0;
		$incoming = $this->_receive_buffer;
		$this->_receive_buffer = '';
		do {
		...
		if ($incoming != "") {
			...
			for ($a = 0; $a < $packet_count; $a++) {
				...
				// if the last packet is invalid, it's likely that it was incomplete (eg: further
				// XML to complete the stanza is forthcoming from the server) so we save it for
				// use in the next call to _receive()
				if ( (!$this->xml->valid) && ($a==$packet_count-1) && (substr($temp[$a],0,14)!='<stream:stream') ) {
					$this->_receive_buffer = $temp[$a];
					$this->_log("RECV (partial): ".$temp[$a]);
				} else {
					$this->_packet_queue[] = $packet;
	...
	// ==== Experimental service discovery support - BEGIN ====================
	// courtesy of noalwin

	// Ask for information about an service
	function get_service_details($service,$node=NULL) {
		if (isset($node)) {
			$node_get_service_details_id = $this->_unique_id("get_service_details_node");
			$this->_set_iq_handler("_on_get_service_details_result",$node_get_service_details_id,NULL,$service,$node);
			
			$xml = "<iq type='get' id='$node_discovery_id' to='$service'>";
			$xml .= "<query xmlns='http://jabber.org/protocol/disco#info' node='$node' />";
			$xml .= "</iq>";
			
			return $this->_send($xml);
		} else {
			$browse_id = $this->_unique_id("get_service_details");
			$this->_set_iq_handler("_on_get_service_details_result",$browse_id,NULL,$service);
	...
	// TODO: the $this->_call_handler("serviceupdate",$jid,$is_new); should include the $node, it will break compatibility?
	function _on_get_service_details_result(&$packet) {
		$packet_type = $this->_node($packet,array('iq','@','type'));
	
		//		$this->_log("GETINFO packet: ".var_export($packet,true));
		...
		if ($packet_type=="result") {
			...
			$jid = $this->_node($packet,array('iq','@','from'));
			
			if ($this->_node($packet,array('iq','#','query',0,'@','node'))) {
				$node = $this->_node($packet,array('iq','#','query',0,'@','node'));
			}
			
			$number_of_identities = is_array($this->_node($packet,array('iq','#','query',0,'#','identity'))) ? count($this->_node($packet,array('iq','#','query',0,'#','identity'))) : 0;
			
			for ($a = 0; $a < $number_of_identities; $a++)
			{
				$identity = $this->_node($packet,array('iq','#','query',0,'#','identity',$a));
				
				if (isset($node)) {
					$this->services[$jid]['nodes'][$node]["identities"][$a] = array(	
						"type"			=> $this->_node($identity,array('@','type')),
						"category"		=> $this->_node($identity,array('@','category'))
					);
					if ($this->_node($identity,array('@','name'))) {
						$this->services[$jid]['nodes'][$node]["identities"][$a]["name"] = $this->_node($identity,array('@','name'));
					}
					...
					if ($this->_node($identity,array('@','name'))) {
						$this->services[$jid]["identities"][$a]["name"] = $this->_node($identity,array('@','name'));
					}
				}
				if ($this->service_single_update) {
					$services_updated = true;
			...
			$number_of_features = is_array($this->_node($packet,array('iq','#','query',0,'#','feature'))) ? count($this->_node($packet,array('iq','#','query',0,'#','feature'))) : 0;
			
			for ($a = 0; $a < $number_of_features; $a++) {
				$feature = $this->_node($packet,array('iq','#','query',0,'#','feature',$a));
				
				if (isset($node)) {				
					$this->services[$jid]['nodes'][$node]["features"][$a] = $this->_node($feature,array('@','var'));
				} else {
					$this->services[$jid]["features"][$a] = $this->_node($feature,array('@','var'));
				}
				if ($this->service_single_update) {
					$services_updated = true;
				} else {
			...
			if ($this->service_single_update && $services_updated) $this->_call_handler("serviceupdate",NULL,$is_new);
			
			$this->_call_handler("servicesupdated",$jid,$node);
	...
	// Get the childs of $parent
	function discover($parent,$node=NULL) {
		if (isset($node)) {
			$node_discovery_id = $this->_unique_id("discover_node");
			$this->_set_iq_handler("_on_discovery_result",$node_discovery_id,NULL,$parent,$node);
			
			$xml = "<iq type='get' id='$node_discovery_id' to='$parent'>";
			$xml .= "<query xmlns='http://jabber.org/protocol/disco#items' node='$node' />";
			$xml .= "</iq>";
			return $this->_send($xml);
		} else {
			
			$discovery_id = $this->_unique_id("discover");
	...
	function _on_discovery_result(&$packet) {
		...
		if ($packet_type=="result") {
			...
			if ($this->_node($packet,array('iq','#','query',0,'@','node'))) {
				$jid_node = $this->_node($packet,array('iq','#','query',0,'@','node'));
			} else {
				$jid_node = NULL;
			}
			
			$number_of_services = is_array($this->_node($packet,array('iq','#','query',0,'#','item'))) ? count($this->_node($packet,array('iq','#','query',0,'#','item'))) : 0;
		
			for ($a = 0; $a < $number_of_services; $a++) {
				$svc = $this->_node($packet,array('iq','#','query',0,'#','item',$a));
				
				$service_jid = strtolower($this->_node($svc,array('@','jid')));
				
				if ($this->_node($svc,array('@','node'))) {
					//It's a hierarchy node, not a service
// 					$is_new = False; //Don't add nodes to roster
					$node = $this->_node($svc,array('@','node'));
...
// 					echo "----------------------\n";
			...
			$this->_call_handler("servicesdiscovered",$childs,$childs_nodes,$jid,$jid_node);
			$this->_log("Received service list");
			//$this->_log("Received service list: ".print_r($this->services,true));
